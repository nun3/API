...
Cypress.Commands.add('waitMessageStatus', (status) => {
    ...
    .contains(status, { timeout: 4000 })
    .then(($el) => {
      expect($el.text()).to.equal(status);
    });
}); 
Cypress.Commands.add('waitMessageStatusInativo', () => {
    cy.get('.card-inativo').should('contain', '(Inativo)', { timeout: 40000 });
}); 
